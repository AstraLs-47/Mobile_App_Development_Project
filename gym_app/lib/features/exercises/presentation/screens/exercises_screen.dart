...
class _ExercisesScreenState extends State<ExercisesScreen> {
  ...
  final List<Map<String, dynamic>> _categories = [
    ...
    {'name': 'Aerobics', 'icon': Icons.music_note},
  ];
              ...
              child: Row(
                ...
                children: [
                  ...
                  Stack(
                    children: [
                      IconButton(
                        icon: const Icon(Icons.notifications_none_outlined, size: 32, color: Colors.black54),
                        onPressed: () {
                          setState(() {
              ...
              child: FutureBuilder<List<Activity>>(
                ...
                builder: (context, snapshot) {
                  if (snapshot.connectionState == ConnectionState.waiting) {
                    return const Center(child: CircularProgressIndicator(color: Color(0xFF0E6CF2)));
                  } else if (snapshot.hasError) {
                    return Center(child: Text('Error: ${snapshot.error}'));
                  } else if (!snapshot.hasData || snapshot.data!.isEmpty) {
                    return const Center(child: Text('No exercises found', style: TextStyle(color: AppColors.textSecondary)));
                  }
                  ...
                  return ScrollConfiguration(
                    ...
                    child: filteredActivities.isEmpty
                      ? const Center(child: Text('No exercises found in this category', style: TextStyle(color: AppColors.textSecondary)))
                      : GridView.builder(
                          physics: const BouncingScrollPhysics(),
                          ...
                          itemBuilder: (context, index) {
                            return _buildAdminStyleUserCard(filteredActivities[index]);
                          },
                        ),
                  );
                },
  ...
  Widget _buildAdminStyleUserCard(Activity workout) {
    return GestureDetector(
      onTap: () => context.pushNamed(RouteConstants.exerciseDetailName, extra: workout.toJson().map((k, v) => MapEntry(k, v.toString()))),
      child: Container(
        decoration: BoxDecoration(
          color: Colors.white,
              ...
              Expanded(
                ...
                child: Container(
                  ...
                  child: Image.asset(
                    workout.image,
                    fit: BoxFit.cover,
                    alignment: (workout.title.toLowerCase().contains('cardio') ?? false) || (workout.title.toLowerCase().contains('running') ?? false)
