...
class _HealthMetricsTabState extends State<HealthMetricsTab> {
  ...
  Widget _buildCurrentMetricHeader(HealthRecord? latest) {
    if (latest == null && _selectedMetricIndex == 0) return const SizedBox.shrink();
        ...
        children: [
          ...
          Row(
            children: [
              ...
              Text(
                latest.bmiCategory,
                style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: latest.bmiColor),
              ),
            ],
          ),
        ],
  ...
  Widget _buildGraphContainer() {
    ...
    final datasets = GraphTransformers.healthRecordsToDatasets(records, _selectedMetricLabel);

    double? interval;
    switch (_selectedMetricLabel) {
      case 'BMI': interval = 7.0; break;
      case 'Heart Rate': interval = 20.0; break;
      case 'BP': interval = 30.0; break;
    }
  ...
  Widget _buildMetricsGrid(HealthRecord? latest) {
    return GridView.count(
      ...
      children: [
        _buildMetricCard(
          latest != null ? '${latest.systolic.toInt()}/${latest.diastolic.toInt()}' : '0/0',
          'mmHg',
          'Blood Pressure',
          Icons.bolt,
        ),
  ...
  Widget _buildMetricCard(String value, String unit, String label, IconData icon) {
    return Container(
